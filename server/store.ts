...
export const ELEVATED_ROLES: Role[] = ['super_admin', 'department_admin', 'moderator', 'supervisor'];
...
// ------------------------------------------------------------------
export async function addPoints(userId: string, type: string, points: number, referenceId?: string, reason?: string) {
...
export async function getHonorBoard(filters?: { departmentId?: string; category?: string; featured?: boolean; limit?: number }) {
  const where: any = {};
...
export async function updateHonorEntry(id: string, data: {
  name?: string;
  achievementTitle?: string;
  category?: string;
  description?: string;
...
export async function listResources(filter: {
  courseId?: string; departmentId?: string; category?: string; status?: string;
  moderationStatus?: string; verificationStatus?: string; uploaderId?: string;
  search?: string; semester?: string; academicYear?: string;
  sortBy?: string; currentUserId?: string; currentUserRole?: string;
  page?: number; limit?: number;
}) {
  const isElevated = filter.currentUserRole && ELEVATED_ROLES.includes(filter.currentUserRole as Role);
  const where: any = {};
  ...
  if (filter.moderationStatus && filter.moderationStatus !== 'all') where.moderationStatus = filter.moderationStatus;
  if (filter.verificationStatus && filter.verificationStatus !== 'all') where.verificationStatus = filter.verificationStatus;
  ...
  if (filter.academicYear && filter.academicYear !== 'all') where.academicYear = filter.academicYear;
  if (filter.search) {
    where.OR = [
  ...
  const orderBy =
    filter.sortBy === 'downloads' ? { downloadsCount: 'desc' as const } :
    filter.sortBy === 'rating' ? { rating: 'desc' as const } :
...
export async function voteResource(id: string, userId: string, voteType: 'helpful' | 'not_helpful') {
  ...
  await prisma.resource.update({ where: { id }, data: { helpfulCount: helpful, notHelpfulCount: notHelpful } });
...
export async function listPosts(filter: {
  courseId?: string; departmentId?: string; postType?: string; category?: string; search?: string;
  sortBy?: string; isSolved?: boolean; currentUserId?: string; page?: number; limit?: number;
}) {
  const where: any = {};
  if (filter.courseId && filter.courseId !== 'all') where.courseId = filter.courseId;
  if (filter.departmentId && filter.departmentId !== 'all') where.departmentId = filter.departmentId;
  ...
  const orderBy =
    filter.sortBy === 'popular' ? [{ upvotes: 'desc' as const }, { createdAt: 'desc' as const }] :
    { createdAt: 'desc' as const };
...
export async function upvotePost(postId: string, userId: string) {
  ...
  if (existing) {
    await prisma.$transaction([
      ...
      prisma.discussionThread.update({ where: { id: postId }, data: { upvotes: { decrement: 1 } } })
    ]);
...
export async function createNotification(n: {
  userId: string; category?: string; type: string; title: string; titleAr?: string;
  message: string; messageAr?: string; actionTab?: string; actionTargetId?: string; metadata?: any;
}) {
...
export async function notifyStaff(n: {
  category?: string; type: string; title: string; titleAr?: string;
  message: string; messageAr?: string; actionTab?: string; actionTargetId?: string;
}) {
  const staff = await prisma.user.findMany({ where: { role: { in: ELEVATED_ROLES } }, select: { id: true } });
...
export async function writeAudit(e: {
  category: string; eventType: string; severity?: string;
  actorId: string; actorName: string; actorRole: string; actorEmail?: string;
  targetId?: string; targetType?: string; targetName?: string;
  previousState?: any; newState?: any; ipAddress: string; userAgent?: string; metadata?: any;
}) {
  try {
...
export async function queryAudit(params: {
  category?: string; eventType?: string; severity?: string; actorId?: string;
  targetId?: string; search?: string; startDate?: string; endDate?: string; page?: number; limit?: number;
}) {
  const where: any = {};
  ...
  if (params.startDate && !isNaN(Date.parse(params.startDate))) where.createdAt = { ...(where.createdAt || {}), gte: new Date(params.startDate) };
  if (params.endDate && !isNaN(Date.parse(params.endDate))) where.createdAt = { ...(where.createdAt || {}), lte: new Date(params.endDate) };
  ...
  return { events: rows, total, page, limit, totalPages: Math.ceil(total / limit) || 1, hasMore: page * limit < total };
}

export async function auditMetrics() {
  const [totalEvents, failedLogins, successfulLogins, roleChanges, aiQueries, criticalEvents] = await Promise.all([
...
export async function platformStats() {
  const [totalUsers, students, approvedFiles, pendingQueue, totalCourses, downloadsAgg] = await Promise.all([
    prisma.user.count(),
