...
const DOWNLOAD_SECRET =
  process.env.DOWNLOAD_SECRET && process.env.DOWNLOAD_SECRET !== "changeme"
...
if (!DOWNLOAD_SECRET) {
  throw new Error(
    "[Security] DOWNLOAD_SECRET/SESSION_SECRET is not set. Download tokens cannot be issued.",
  );
}
...
function verifySignedDownloadToken(fileId: string, token: string): boolean {
  ...
  const expected = crypto
    ...
    .update(`${fileId}:${expiresTimestamp}`)
    .digest("hex");
  try {
...
async function getSessionUser(req: Request) {
  ...
  const token =
    cookies.enghub_session || (authHeader ? authHeader.replace(/^Bearer\s+/i, "").trim() : "");
...
const _requireElevated = requireRole([
  "super_admin",
  "department_admin",
  "supervisor",
  "moderator",
...
function setAuthCookie(res: Response, token: string) {
  ...
  res.setHeader(
    "Set-Cookie",
    `enghub_session=${token}; HttpOnly; Path=/; Max-Age=${7 * 24 * 3600}; SameSite=Lax${secure}`,
  );
}
...
function clearAuthCookie(res: Response) {
  ...
  res.setHeader(
    "Set-Cookie",
    `enghub_session=; HttpOnly; Path=/; Expires=Thu, 01 Jan 1970 00:00:00 GMT; SameSite=Lax${secure}`,
  );
}
...
async function startServer() {
  ...
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    res.on("finish", () => {
      ...
      if (duration > 1000)
        console.warn(
          `[SLOW_REQ] ${req.method} ${req.originalUrl} took ${duration}ms (status: ${res.statusCode})`,
        );
    });
    next();
  });
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!rate.allowed) {
        ...
        return next(
          new RateLimitError(
            `Too many registration attempts. Please wait ${rate.remainingSeconds} seconds.`,
            rate.remainingSeconds,
          ),
        );
      }

      try {
        const { fullName, email, phoneNumber, password, passwordConfirm, departmentId, level } =
          req.body;
        ...
        const defaultEnrolled = isMtr
          ? mtrCourses
          : level === "Year 1 (Freshman)" || level === "المستوى الصفري {إعدادي}"
            ? freshmanCourses
            : sophomoreCourses;
        ...
        const user = await prisma.user.create({
          data: {
            ...
            badges: [
              {
                id: "badge-welcome",
                name: "Engineering Pioneer",
                description: "Joined the Faculty of Engineering Portal.",
                icon: "GraduationCap",
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!rate.allowed) {
        return next(
          new RateLimitError(
            `Too many failed login attempts. Please retry in ${rate.remainingSeconds} seconds.`,
            rate.remainingSeconds,
          ),
        );
      }
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        const cookies = parseCookies(req);
        const currentToken =
          cookies.enghub_session || req.headers.authorization?.replace("Bearer ", "");
  ...
  const handleRevokeSession = (req: Request, res: Response, next: NextFunction) => {
    getSessionUser(req)
      .then(async (user) => {
        ...
        res.json({
          success: revoked,
          message: revoked ? "Session terminated successfully." : "Session not found.",
        });
      })
      .catch(next);
  };
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (notif.userId !== user.id) {
          ...
          return next(
            new ForbiddenError(
              "Forbidden: You cannot modify notifications belonging to another user.",
            ),
          );
        }
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = await getSessionUser(req);
        if (!user) return next(new UnauthorizedError("Authentication session required."));
        const res2 = await prisma.notification.updateMany({
          where: { userId: user.id, read: false },
          data: { read: true },
        });
        res.json({
  ...
  app.get("/api/resources", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await store.listResources({
        ...
        departmentId: q.departmentId as string,
        category: q.category as string,
        semester: q.semester as string,
        academicYear: q.academicYear as string,
        verificationStatus: q.verificationStatus as string,
        moderationStatus: q.moderationStatus as string,
        uploaderId: q.uploaderId as string,
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!isElevated && !allowedStudentCategories.includes(category)) {
          return next(
            new ForbiddenError(
              "غير مصرح للطلاب برفع هذا النوع من الملفات. يُسمح للطلاب فقط برفع الملخصات والقوانين المركزة وتخضع للتدقيق والاعتماد من الإشراف الأكاديمي.",
            ),
          );
        }
        ...
        if (buffer.length > 30 * 1024 * 1024)
          return next(new BadRequestError("File exceeds 30MB limit."));
        ...
        if (!validation.valid) {
          await store.writeAudit({
            ...
            actorRole: user.role,
            ipAddress: getClientIp(req),
            metadata: { fileName, declaredExtension: ext, error: validation.error },
          });
          ...
          data: {
            ...
            uploaderDepartment:
              user.departmentId === "dept-mtr"
                ? "هندسة الميكاترونكس"
                : "هندسة الحاسب والذكاء الاصطناعي",
        ...
        await store.createNotification({
          ...
          title: isElevated
            ? "Resource Published Instantly"
            : "Resource Submitted to Moderation Queue",
          titleAr: isElevated
            ? "تم نشر الملف الأكاديمي مباشرة"
            : "تم إرسال الملخص لطابور المراجعة والتدقيق",
          message: isElevated
            ? `Your official resource "${resource.title}" is now live.`
            : `Your summary "${resource.title}" will be reviewed by supervisors (+15 pts upon approval).`,
          messageAr: isElevated
            ? `تم نشر مرجعك الأكاديمي المعتمد "${resource.title}" بنجاح.`
            : `تم استلام ملخصك "${resource.title}" وسيتم اعتماده قريباً (+15 نقطة فور الموافقة).`,
          actionTab: "courses",
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!store.ELEVATED_ROLES.includes(user.role as any)) {
          return next(
            new ForbiddenError("Access forbidden: Elevated moderator or admin role required."),
          );
        }
        ...
        if (
          ...
          return next(
            new ForbiddenError(
              "Department Admins can only moderate resources within their department.",
            ),
          );
        }
        ...
        if (action === "approve") {
          ...
          if (resource.uploaderId !== user.id) {
            await store.addPoints(
              ...
              15,
              resource.id,
              `Approved resource "${resource.title}"`,
            );
          }
          await store.createNotification({
          ...
          if (resource.uploaderId !== user.id) {
            await store.addPoints(
              ...
              25,
              resource.id,
              `Verified resource "${resource.title}"`,
            );
          }
          await store.createNotification({
            userId: resource.uploaderId,
          ...
          if (!rejectionReason || rejectionReason.trim().length < 5) {
            return next(
              new BadRequestError("A specific rejection reason is mandatory when rejecting."),
            );
          }
        ...
        await store.writeAudit({
          ...
          targetName: resource.title,
          previousState,
          newState: { status: updated.status, moderationStatus: updated.moderationStatus },
          ipAddress: getClientIp(req),
        });

        res.json({
          success: true,
          resource: store.toSafeResource(updated, user.id),
          message: `تم اتخاذ القرار (${action}) بنجاح.`,
        });
      } catch (err) {
        next(err);
      }
  ...
  app.post(
    ...
    validate({
      params: { id: Validators.string(1, 100) },
      body: { voteType: Validators.enum(["helpful", "not_helpful"]) },
    }),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = await getSessionUser(req);
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        res.status(201).json({
          success: true,
          post: store.toSafePost(safe, user.id, false),
  ...
  app.post(
    ...
    validate({
      params: { id: Validators.string(1, 100) },
      body: { content: Validators.string(3, 3000) },
    }),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = await getSessionUser(req);
  ...
  app.post(
    ...
    validate({
      params: { id: Validators.string(1, 100) },
      body: { commentId: Validators.string(1, 100) },
    }),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = await getSessionUser(req);
        ...
        if (!isAuthor && !isElevated) {
          return next(
            new ForbiddenError("Only the post author or a moderator can verify solutions."),
          );
        }
        ...
        await store.writeAudit({
          ...
          targetType: "post",
          targetName: post.title,
          newState: { commentId: comment.id, solverId: comment.authorId },
          ipAddress: getClientIp(req),
        });
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        res
          .status(201)
          .json({ success: true, entry, message: "تم إضافة الطالب إلى لوحة الشرف بنجاح." });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!rate.allowed)
        return next(
          new RateLimitError(
            `AI Assistant rate limit reached. Please wait ${rate.remainingSeconds}s.`,
            rate.remainingSeconds,
          ),
        );
...
- Do NOT mention model names, version numbers, or provider names.`;
      ...
      if (!process.env.GEMINI_API_KEY) {
        if (wantStream) {
          ...
          res.write(
            `data: ${JSON.stringify({ error: errorMessageArabic, text: errorMessageArabic })}\n\n`,
          );
          res.write("data: [DONE]\n\n");
          return res.end();
        }
...
- "explanation": string`;
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!quiz) {
          return next(
            new BadRequestError(
              "Unknown quiz. Grading requires a server-registered quiz bank entry.",
            ),
          );
        }
        ...
        const pointsEarned = user
          ? passed
            ? Math.min(25, Math.max(10, Math.round(percentage / 5)))
            : 5
          : 0;
        ...
        if (user && pointsEarned > 0) {
          await store.addPoints(
            ...
            pointsEarned,
            quizId,
            `Quiz "${quiz.title}" score ${percentage}%`,
          );
        }
  ...
  app.post("/api/study/pomodoro/log", async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const session = await prisma.pomodoroSession
        .create({
          data: {
      ...
      res.json({
        ...
        message:
          pointsEarned > 0
            ? `تم تسجيل جلسة التركيز وإضافة +${pointsEarned} نقاط!`
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        next(new BadRequestError(err.message || "Failed to create course"));
      }
    },
  );
  ...
  app.patch(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        next(new BadRequestError(err.message || "Failed to update course"));
      }
    },
  );
  ...
  app.delete(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        next(new BadRequestError(err.message || "Failed to archive course"));
      }
    },
  );
  ...
  app.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        res
          .status(201)
          .json({ success: true, exam: saved, message: "تم حفظ الاختبار في بنك الأسئلة بنجاح." });
      } catch (err) {
        next(err);
      }
    },
  ...
  app.get(
    "/api/admin/stats",
    requireRole(["super_admin", "department_admin", "moderator"]),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        const user = (req as any).user;
  ...
  app.get(
    "/api/admin/users",
    requireRole(["super_admin", "department_admin", "moderator", "supervisor"]),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        res.json({
          users: rows.map(store.toSafeUser),
          total,
          page,
          limit,
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (newRole === "super_admin" && user.role !== "super_admin") {
          return next(
            new ForbiddenError("Only super administrators may assign the super_admin role."),
          );
        }
        if (
          user.role === "department_admin" &&
          ["super_admin", "department_admin"].includes(newRole)
        ) {
          return next(
            new ForbiddenError(
              "Department administrators may not assign super_admin or department_admin roles.",
            ),
          );
        }
        ...
        if (!targetUser)
          return next(
            new NotFoundError("Target user does not exist. Accounts must register first."),
          );

        if (user.role === "department_admin" && targetUser.departmentId !== user.departmentId) {
          return next(
            new ForbiddenError(
              "Department administrators may only modify users within their own department.",
            ),
          );
        }
        ...
        if (supervisorScope !== undefined) {
          try {
            data.supervisorScope = JSON.parse(supervisorScope);
          } catch {
            data.supervisorScope = supervisorScope;
          }
        }
        ...
        res.json({
          success: true,
          message: `User ${updated.name} (${updated.email}) role updated to ${newRole}.`,
          user: store.toSafeUser(updated),
        });
      } catch (err) {
        next(err);
  ...
  app.get(
    ...
    async (_req: Request, res: Response, next: NextFunction) => {
      try {
        res.json({
          success: true,
          metrics: await store.auditMetrics(),
  ...
  app.get(
    ...
    validate({ query: { fileId: Validators.string(1, 100) } }),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (resource) {
          ...
          if (resource.moderationStatus !== "approved" && !isElevated && !isOwner) {
            return next(
              new ForbiddenError(
                "This resource is pending moderation review and cannot be downloaded yet.",
              ),
            );
          }
        }
  ...
  app.get(
    ...
    validate({
      params: { fileId: Validators.string(1, 100) },
      query: { token: Validators.string(5, 500) },
    }),
    async (req: Request, res: Response, next: NextFunction) => {
      try {
        ...
        if (!verifySignedDownloadToken(fileId, req.query.token as string)) {
          return next(
            new ForbiddenError(
              "Invalid or expired download signature. Please request a fresh signed link.",
            ),
          );
        }
        const resource = await store.getResourceById(fileId);
