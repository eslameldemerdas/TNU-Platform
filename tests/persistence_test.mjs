...
async function main() {
  ...
  log('fetch departments', !!deptId, `deptId=${deptId}`);
  ...
  // Verify via API (simulates page refresh / different browser)
  ...
  log('course: gone from DB after delete', deletedCourse?.archivedAt !== null, `archivedAt=${deletedCourse?.archivedAt}`);
  ...
  const ancInApi = (Array.isArray(ancApiR.json) ? ancApiR.json : ancApiR.json?.announcements || []).find((a: any) => a.id === ANC_ID);
  log('announcement: visible via API', !!ancInApi, `title=${ancInApi?.title}`);
  ...
  const examR = await req('POST', '/api/exams', {
    cookie: ADMIN_COOKIE,
    body: { courseId: COURSE_ID || 'course-eng011', courseCode: 'PERSIST', title: 'Persistence Exam', topic: 'Test', durationMinutes: 60, totalMarks: 100, difficulty: 'Medium', term: 'Quiz', departmentId: deptId, questions: [{ id: 'q1', question: 'Q1?', options: ['A', 'B'], correctIndex: 0 }] },
  });
  const EXAM_ID = examR.json?.exam?.id;
  log('exam: create', examR.status === 201 && !!EXAM_ID, `id=${EXAM_ID}`);
  ...
  const resR = await req('POST', '/api/resources', {
    cookie: ADMIN_COOKIE,
    body: { title: 'Persistence Resource', description: 'Test', category: 'summary', courseId: COURSE_ID || 'course-eng011', courseCode: 'PERSIST', departmentId: deptId, fileType: 'pdf', fileName: `persist-${RUN}.pdf`, fileData: b64, tags: ['persist'] },
  });
  const RES_ID = resR.json?.resource?.id;
  log('resource: create', resR.status === 201 && !!RES_ID, `id=${RES_ID}`);
