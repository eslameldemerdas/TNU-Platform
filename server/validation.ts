...
export const Validators = {
  string: (min = 1, max = 2000): ValidatorFn => (value, field) => {
  ...
  password: (min = 8): ValidatorFn => (value, field) => {
  ...
  enum: (allowedValues: readonly string[]): ValidatorFn => (value, field) => {
  ...
  integer: (min = 0, max = Number.MAX_SAFE_INTEGER): ValidatorFn => (value, field) => {
  ...
  array: (itemValidator?: ValidatorFn, minLen = 0, maxLen = 100): ValidatorFn => (value, field) => {
  ...
  number: (min = -Number.MAX_SAFE_INTEGER, max = Number.MAX_SAFE_INTEGER): ValidatorFn => (value, field) => {
    const num = Number(value);
  ...
  object: (_schema?: Record<string, ValidatorFn>): ValidatorFn => (value, field) => {
  ...
  optional: (validator: ValidatorFn): ValidatorFn => (value, field) => {
