...
async function runRbacTests() {
  ...
  assert(res1.status === 401 || res1.status === 403, 'GET /api/admin/stats as unauthenticated user is rejected with 401/403', `Status: ${res1.status}`);
  ...
  assert(res2.status === 401 || res2.status === 403, 'GET /api/admin/users as unauthenticated user is rejected with 401/403', `Status: ${res2.status}`);
  ...
  assert(studentLoginRes.status === 200, 'Student login succeeds', `Status: ${studentLoginRes.status}`);
  ...
  assert(res4.status === 403, 'GET /api/admin/stats with Student session returns 403 Forbidden', `Status: ${res4.status}`);
  ...
  assert(res5.status === 403, 'GET /api/admin/users with Student session returns 403 Forbidden', `Status: ${res5.status}`);
  ...
  assert(adminLoginRes.status === 200, 'Super Admin login succeeds', `Status: ${adminLoginRes.status}`);
  const adminCookie = adminLoginRes.headers.get('set-cookie') || '';
  const adminLoginData = await adminLoginRes.json();
  assert(adminLoginData.user?.role === 'super_admin', 'Authenticated user role is super_admin', `Role: ${adminLoginData.user?.role}`);
  ...
  assert(res7.status === 200, 'GET /api/admin/stats as Super Admin returns 200 OK', `Status: ${res7.status}`);
  const statsData = await res7.json();
  assert(typeof statsData.totalUsers === 'number', 'Stats payload contains totalUsers metric', `Total Users: ${statsData.totalUsers}`);
  ...
  assert(res8.status === 200, 'GET /api/admin/users as Super Admin returns 200 OK', `Status: ${res8.status}`);
  const usersData = await res8.json();
  assert(Array.isArray(usersData.users) && usersData.users.length > 0, 'User directory returned users list', `User count: ${usersData.users?.length}`);
  ...
  assert(res9.status === 200, 'POST /api/admin/update-role as Super Admin returns 200 OK', `Status: ${res9.status}`);
