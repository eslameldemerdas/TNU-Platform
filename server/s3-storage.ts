...
export function getS3StorageConfig(): StorageConfig | null {
  ...
  const bucket = process.env.S3_BUCKET_NAME || process.env.AWS_S3_BUCKET || process.env.R2_BUCKET_NAME;
  const region = process.env.S3_REGION || process.env.AWS_REGION || 'auto';
  const endpoint = process.env.S3_ENDPOINT || process.env.R2_ENDPOINT || (process.env.R2_ACCOUNT_ID ? `https://${process.env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com` : undefined);
 ...
 */
export function generateEncryptedFileName(originalFileName: string, courseId: string): { fileKey: string; secureFileName: string } {
  const ext = originalFileName.includes('.')
    ? '.' + originalFileName.split('.').pop()!.toLowerCase().replace(/[^a-z0-9]/g, '')
    : '.pdf';

  const uuid = crypto.randomUUID();
  const hash = crypto.createHash('sha256').update(`${uuid}:${originalFileName}:${Date.now()}`).digest('hex').substring(0, 16);
...
export async function getPresignedDownloadUrl(params: {
  ...
  const expiresAt = new Date(Date.now() + expiresIn * 1000).toISOString();
  const safeFilename = params.originalFileName ? params.originalFileName.replace(/["\r\n]/g, '') : 'document.pdf';

  if (!config) {
    const secret = process.env.SESSION_SECRET;
    if (!secret) {
      throw new Error('[Security] SESSION_SECRET is not set. Cannot generate signed download URLs.');
    }
    const signature = crypto.createHmac('sha256', secret).update(`${params.fileKey}:${expiresAt}`).digest('hex');
