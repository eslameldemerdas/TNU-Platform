...
export type PointsLedgerType =
  "upload_approved" | "answer_accepted" | "helpful_comment" | "moderation_removal" | "bonus";
...
export function verifySignedDownloadToken(fileId: string, token: string): boolean {
  ...
  const expectedSignature = crypto
    .createHmac("sha256", DOWNLOAD_HMAC_SECRET)
    .update(payload)
    .digest("hex");
...
export class DatabaseEngine {
  ...
  public courses = new Map<
    string,
    Course & { archivedAt?: string | null; category?: CourseCategory }
  >();
  ...
  private seedRelationalData() {
    ...
    const adminEmail = (process.env.ADMIN_EMAIL || "eldmrdasheslam1@gmail.com")
      .trim()
      .toLowerCase();
    ...
    this.addPointsLedgerEntryInternal(
      studentAlex.id,
      "upload_approved",
      50,
      "res-01",
      "Approved lecture summary notes",
    );
    this.addPointsLedgerEntryInternal(
      studentAlex.id,
      "answer_accepted",
      30,
      "post-01",
      "Accepted solution in Math discussion",
    );
    this.addPointsLedgerEntryInternal(
      studentAlex.id,
      "bonus",
    ...
    const welcomeNotif: StoredNotification = {
      ...
      messageAr:
        "تم إعداد حسابك الأكاديمي بنجاح. يمكنك الآن تصفح المقررات وتحميل المذكرات المعتمدة.",
      read: false,
      createdAt: now,
    };
  ...
  // --------------------------------------------------------------------------
  private addPointsLedgerEntryInternal(
    userId: string,
    type: PointsLedgerType,
    points: number,
    referenceId?: string,
  ...
  public getCourseById(
    id: string,
  ): (Course & { archivedAt?: string | null; category?: CourseCategory }) | null {
  ...
  public getResources(filter: {
    ...
    const isElevated =
      ...
      ["super_admin", "department_admin", "moderator", "supervisor"].includes(
        filter.currentUserRole,
      );
    ...
    return {
      resources: paginated.map((r) => this.toSafeResource(r, filter.currentUserId)),
      total,
      page,
      limit,
      totalPages: Math.ceil(total / limit),
  ...
  // --------------------------------------------------------------------------
  public queryPosts(filter: {
    courseId?: string;
    departmentId?: string;
    postType?: string;
    category?: string;
    search?: string;
    sortBy?: "recent" | "popular" | "unsolved";
    isSolved?: boolean;
    currentUserId?: string;
    page?: number;
    limit?: number;
  ...
  public getPosts(filter: {
    ...
    return {
      posts: paginated.map((p) => this.toSafePost(p, filter.currentUserId)),
      total,
      page,
      limit,
      totalPages: Math.ceil(total / limit),
