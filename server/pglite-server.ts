...
export async function startEmbeddedDb(): Promise<void> {
  ...
  if (alreadyUp) {
    console.log(
      `[EmbeddedDB] Reusing existing PostgreSQL bridge on ${EMBEDDED_HOST}:${EMBEDDED_PORT}`,
    );
    return;
  }
  ...
  await server.start();
  console.log(
    `[EmbeddedDB] PostgreSQL (PGlite) listening on ${EMBEDDED_HOST}:${EMBEDDED_PORT}, data dir: ${dataDir}`,
  );
}

/** Standalone runner so `prisma db push` / seed can talk to the embedded DB. */
const isDirectRun =
  process.argv[1] && process.argv[1].replace(/\\/g, "/").includes("pglite-server");
if (isDirectRun) {
