...
export function isSupervisor(user: UserProfile | null): boolean {
  if (!user) return false;
  return (
    user.role === "supervisor" || user.role === "department_admin" || user.role === "super_admin"
  );
}
...
export function getSupervisorScopeLabel(
  ...
  if (scope.departmentId !== "all") {
    const foundDept = departments.find(
      (d) =>
        d.id === scope.departmentId ||
        d.id === `${scope.departmentId}-01` ||
        scope.departmentId.includes(d.id),
    );
    if (foundDept) {
