...
export async function initiateUpload(req: InitiateUploadRequest): Promise<InitiateUploadResponse> {
  ...
  if (!categoryConfig.mimeTypes.includes(req.fileType)) {
    throw new Error(
      `MIME type '${req.fileType}' is not allowed for category '${req.resourceCategory}'`,
    );
  }
...
export async function verifyUploadedObjectHeader(
  ...
  try {
    ...
    throw new Error(
      `Failed to perform post-upload verification on object (${fileKey}): ${error.message}`,
    );
  }
}
...
export interface ConfirmUploadInput {
  ...
  description?: string;
  resourceType:
    "lecture" | "section" | "lab" | "assignment" | "previousExam" | "summary" | "importantQuestion";
  fileKey: string;
  fileUrl: string;
  fileSize: number;
  fileType: string;
